...
const DeleleButton = ({ film }) => {
  const { deleteFilm } = useContext(FilmContext);
  return <span  onClick={() => deleteFilm(film)}  className="ui red basic button">
...
const FilmCardButtons = ({ film }) => {
  ...
  const confirmButtons = <div className="ui two buttons">
    <DeleleButton film={film} />
     <span onClick={hideConfirm} className="ui grey basic button">
      <i className="ui icon close" /> NO
    </span>
  </div>

    const buttons = <div className="ui two buttons">
