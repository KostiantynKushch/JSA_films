...
class FilmsPage extends Component {
	...
	updateFilm = filmData => api.films.update(filmData).then(film => this.setState(({ films }) => ({
		films: this.sortFilms(films.map(f => (f._id === film._id ? film : f))),
	})));

	saveFilm = film => (film._id ? this.updateFilm(film) : this.addFilm(film));

	deleteFilm = film => api.films.delete(film).then(() => this.setState(({ films }) => ({
			...
			<FilmContext.Provider value={this.state}>
				<div className="ui stackable grid">
					<div className="six wide column">
						...
						<Route path="/films/edit/:_id" render={({ match }) => <FilmForm
